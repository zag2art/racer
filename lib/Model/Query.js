  ...
  model.on('all', function(segments) {
    ...
    for (var hash in map) {
      ...
      if (query.isPathQuery && query.shareQuery && util.mayImpact(query.expression, segments)) {
        ...
        currQuery = query.pathQuery;
        updatedQuery = query.model._get(query.expression);

        if (!util.deepEqual(currQuery, updatedQuery)) query.shareQuery.setQuery(query.sourceQuery());
      }
    }
  });
});
...
Model.prototype._initQueries = function(items) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    var source = item[6];
    var extra = item[7];
    var initialPathQuery = item[8];
    var query = new Query(this, collectionName, expression, source, initialPathQuery);
    queries.add(query);
...
function Query(model, collectionName, expression, source, initialPathQuery) {
  ...
  this.initialPathQuery = initialPathQuery;
  this.pathQuery = null;
...
Query.prototype.sourceQuery = function() {
  ...
  if (this.isPathQuery) {
    if (this.initialPathQuery) {
      this.pathQuery = this.initialPathQuery;
      delete this.initialPathQuery;
    } else {
      this.pathQuery = pathQuery(this.model, this.expression);
    }
...
function pathQuery(model, segments){
  var value = model._get(segments);

  if (util.isPlainObject(value)) return util.deepCopy(value);
  if (!Array.isArray(value)) value = [value];
  return {_id: {$in: value.slice()}};
}
