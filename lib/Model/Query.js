...
var defaultType = require('sharedb').types.defaultType;
...
Model.INITS.push(function(model) {
  ...
  model.on('all', function(segments) {
    ...
    for (var hash in map) {
      var query = map[hash];
      var expression = model._splitPath(query.expression);
      if (!query.isLiveQuery) continue;
      if (!query.shareQuery) continue;
      if (!util.mayImpact(expression, segments)) continue;

      var oldQuery = query.shareQuery.query;
      var newQuery = query.model._get(expression);

      if (util.deepEqual(oldQuery, newQuery)) continue;

      query.shareQuery.setQuery(query.prepareQuery());
    }
  });
});
...
Model.prototype._initQueries = function(items) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    var options = item[5];
    var extra = item[6];
    var serializedQuery = item[7];
    var query = new Query(this, collectionName, expression, options);
    query._serializedLiveQuery = serializedQuery;
    queries.add(query);
...
function Query(model, collectionName, expression, options) {
  ...
  this.isLiveQuery = typeof expression === 'string';
...
Query.prototype.prepareQuery = function() {
  var query;

  if (this.isLiveQuery) {
    if (this._serializedLiveQuery) {
      query = this._serializedLiveQuery;
      delete this._serializedLiveQuery;
    } else {
      query = util.deepCopy(this.model.get(this.expression));
    }
    return query;
  }
...
Query.prototype._getShareResults = function() {
  ...
  for (var i = 0; i < ids.length; i++) {
    ...
    if (doc) results.push(doc.shareDoc);
  }
  return results;
};
...
Query.prototype.serialize = function() {
  ...
  , this.options
  , this.getExtra()
  , this.shareQuery && this.shareQuery.query
  ];
