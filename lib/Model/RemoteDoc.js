...
RemoteDoc.prototype._updateCollectionData = function() {
  ...
  if (snapshot) snapshot.id = this.id;
...
RemoteDoc.prototype.set = function(segments, value, cb) {
  if (segments.length === 0 && !this.shareDoc.type) {
    ...
    // We deep copy the snapshot at time of create to prevent the id added
    // outside of ShareJS from getting stored in the data
    this.shareDoc.snapshot = util.deepCopy(value);
    this._updateCollectionData();
    return;
  }
...
RemoteDoc.prototype._onOp = function(op) {
  ...
  // StringInsertOp
    ...
    // DEPRECATED: The separate stringInsert event will be removed in lieu
    // of the extra parameters in passed to the change listener
    ...
    var pass = model.pass({$original: 'stringInsert', index: index, text: text, previous: previous})._pass;
  ...
  // StringRemoveOp
    ...
    segments = segments.slice(0, -1);
    // DEPRECATED: The separate stringRemove event will be removed in lieu
    // of the extra parameters in passed to the change listener
    model.emit('stringRemove', segments, [index, howMany, model._pass]);
    var value = model._get(segments);
    var previous = value.slice(0, index) + text + value.slice(index);
    var pass = model.pass({$original: 'stringRemove', index: index, howMany: howMany, previous: previous})._pass;
    model.emit('change', segments, [value, previous, pass]);
