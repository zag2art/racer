...
RemoteDoc.prototype.stringInsert = function(segments, index, value, cb) {
  ...
  if (previous == null) {
    var lastSegment = segments[segments.length - 1];
    if (util.isArrayIndex(lastSegment)) {
      // undefined - no such array item exists yet so we add a new one
      if (previous === undefined) {
        var op = [new ListInsertOp(segments.slice(0, -1), lastSegment, value)];
      // null - there is an empty array item so we replace it
      } else
        var op = [new ListReplaceOp(segments.slice(0, -1), lastSegment, previous, value)];
      }
    } else
